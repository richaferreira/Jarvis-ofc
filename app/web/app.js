...
let speaking = false, speechGeneration = 0;
function presence(state) {
  if (!$('presence')) return;
  $('presence').dataset.state = state;
  $('presence-state').textContent = ({idle:'Pronto para conversar',listening:'Estou ouvindo você',thinking:'Processando seu pedido',responding:'Recebendo a resposta',speaking:'Falando com você',error:'Não consegui concluir. Confira o aviso.'})[state];
}
function restPresence() { presence(speaking ? 'speaking' : busy ? 'thinking' : voiceActive ? 'listening' : 'idle'); }
function speakAnswer(text) {
  ...
  utterance.voice = voices.find(v => v.lang.replace('_','-').toLowerCase() === 'pt-br') || null;
  utterance.onstart = () => { if(generation===speechGeneration){speaking=true;presence('speaking');} };
  const finish = () => { if(generation===speechGeneration){speaking=false;restPresence();} };
  utterance.onend = finish; utterance.onerror = finish;
  speechSynthesis.speak(utterance);
}
$('avatar-talk').addEventListener('click',()=>{ $('continuous').checked=true; $('speak').checked=true; $('mic').click(); });
$('avatar-stop').addEventListener('click',()=> $('stop').click());
...
$('chat-form').addEventListener('submit', async e => {
  ...
  stopResponse();
  const current = epoch; busy = true; presence('thinking'); $('model-select').disabled = true; $('send').disabled = true; $('clear').disabled = true;
  ...
  try {
    ...
    data.warnings.forEach(event); event('Resposta recebida.'); void loadHistory();
  ...
  } catch (error) { if (current === epoch) { const message = error.name === 'AbortError' ? 'Resposta interrompida.' : error.message; bubble(message, 'assistant', true); event(message); stopResponse(); presence(error.name==='AbortError'?'idle':'error'); } }
  finally { busy = false; if($('presence').dataset.state!=='error') restPresence(); activeRequest = null; $('model-select').disabled = false; $('send').disabled = false; $('clear').disabled = false; $('chat-state').textContent = 'Enter envia · Shift + Enter quebra a linha'; }
});
...
function stopResponse() {
  activeRequest?.abort(); speechGeneration++; speaking=false; restPresence();
...
async function streamChat(text,current) {
  ...
  const started=performance.now(); let firstText=false, answerSpoken=false;
  $('response-time').textContent='Aguardando o primeiro texto…';
  ...
  try {
    while(true) {
      ...
      while((index=pending.indexOf('\n'))>=0) {
        ...
        if(message.type==='token') {if(!firstText){firstText=true;$('response-time').textContent='Primeiro texto em '+((performance.now()-started)/1000).toFixed(1)+' s';} if(!speaking)presence('responding');content.textContent+=message.text;$('messages').scrollTop=$('messages').scrollHeight;}
        if(message.type==='answer' || message.type==='done') {content.textContent=message.text;if(!answerSpoken){answerSpoken=true;speakAnswer(message.text);}}
...
if (Recognition) {
  recognition = new Recognition(); recognition.lang = 'pt-BR'; recognition.interimResults = true;
  recognition.onstart = () => { if(voiceActive && !busy && !speaking)presence('listening'); };
  recognition.onspeechstart = () => { if(voiceActive){stopResponse();presence('listening');} };
  recognition.onresult = async e => {
    const result=e.results[e.resultIndex]; if(!voiceActive)return;
    $('live-transcript').textContent=result[0].transcript;
    if(!result.isFinal)return;
    $('prompt').value = result[0].transcript;
  ...
  recognition.onerror = e => { if(e.error!=='no-speech') {voiceActive=false;presence('error');notice('Reconhecimento interrompido. Confira a permissão do microfone.');} };
  recognition.onend = () => {
  ...
  $('mic').addEventListener('click', () => {
    ...
    try { stopResponse(); voiceActive=true; recognition.start(); presence('listening'); $('mic').textContent='Ouvindo · use Interromper para parar'; } catch { notice('Não foi possível iniciar o microfone.'); }
  });
} else { $('mic').disabled = true; $('continuous').disabled=true; $('mic').textContent = 'Ditado não suportado'; $('avatar-talk').disabled=true; $('avatar-talk').textContent='Voz não suportada neste navegador'; }
...
let lastFrame=-100;
function draw(ms) {
  if(!reduced && (document.hidden || ms-lastFrame<33)){requestAnimationFrame(draw);return;}
  lastFrame=ms;
  ...
  const points = [];
  for (let i=0;i<80;i++) { const y=1-2*i/79, a=i*2.399963+angle, c=Math.sqrt(1-y*y); const x=Math.cos(a)*c,z=Math.sin(a)*c; points.push({x:w/2+x*r,y:h/2+y*r,z}); }
  ctx.lineWidth=.5;
